...
plot_sensor <- function(data, var_ref_id, date_range, sensor_calibrations = NULL,
  ...
  p <- ggplot() +
    geom_scattermore(data = df, aes(x = datetime, y = qc_value, 
                                    colour = !!sym(colour))) +
    facet_wrap(~var_ref_id, scales = "free_y", ncol = 1) +
    labs(colour = "Device ID", linetype = "Sensor events") +
    theme_bw() +
...
plot_var_ts_qc <- function(data, var_ref_id, FUN = median, days_offset = 184) {
  sel_vars <- var_ref_id
...
remove_buoy_out <- function(data, site_events) {
  
  site_dates <- site_events |> 
    ...
    dplyr::mutate(date_interval = lubridate::interval(deployed, removed)) 
...
viz_data <- function(data, long = TRUE, variable = "var_ref_id",
                     site_events = NULL,
                     value = "qc_value") {
  ...
  # Run the app using runGadget, ensuring it works properly in RStudio Viewer
  logs <- shiny::runGadget(shiny::shinyApp(ui, server), viewer = shiny::paneViewer())
  return(logs)
}
