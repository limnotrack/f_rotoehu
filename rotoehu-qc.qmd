...
sensor_map <- site_devices |> 
  ...
  mutate(var_ref_id = generate_var_ref(variable, z_relative, reference)) 
sensor_map
...
raw_long_device <- sensor_map |> 
  ...
  select(site, datetime, device_id, variable, var_ref_id, raw_value, qc_value, 
         flag, qc_code)
...
for (v in sel_vars) {
  ...
  cat('## ', paste(var_name$variable, var_name$value_m, "m"), '\n\n')
...
ph_count <- raw_long_device |> 
  filter(variable %in% c("c_phraw", "c_ph")) |>
  apply_adjustment("c_phraw_d100", FUN = \(x) x / 10) |> 
...
ph_raw <- ph_count |> 
  filter(variable %in% c("c_phraw")) |> 
  mutate(new_val = qc_value) |> 
  select(datetime, new_val)

ph <- ph_count |> 
  filter(variable %in% c("c_ph")) |> 
...
ph_dc <- ph #ph_count |>
  ...
  # mutate(var_ref_id = "c_ph_d100", variable = "c_ph") 
...
fluor_df <- raw_long_device_filt |> 
  filter(variable %in% c("f_chl", "f_phyc")) |> 
...
# viz_data(data = fluor_df, variable = "var_ref_id",
#          site_events = site_events,
#          raw_value = "qc_value")
...
# viz_data(data = phyc_corr, variable = "var_ref_id",
#          site_events = site_events,
#          value = "qc_value")
```
...
do_conc2_1m <- left_join(do_1m, temp_1m, by = "datetime") |> 
  ...
  select(datetime, var_ref_id, site, device_id, variable, qc_value, flag)

do_conc_1m <- raw_long_device_filt |> 
  filter(var_ref_id == "c_do_d100") |> 
  select(datetime, var_ref_id, site, device_id, variable, raw_value)
...
do_conc2_1m <- do_conc2_1m |> 
  ...
  select(datetime, var_ref_id, site, device_id, variable, qc_value,
         flag)

do_conc3_1m <- left_join(do_conc_1m, do_conc2_1m, by = c("datetime", "var_ref_id", "site", "device_id", "variable")) |> 
  mutate(
    qc_code = case_when(
...
do_conc2_10m <- left_join(do_10m, temp_10m, by = "datetime") |> 
  ...
  select(datetime, var_ref_id, site, device_id, variable, qc_value, qc_code,
         flag)
...
do_conc3_10m <- left_join(do_conc_10m, do_conc2_10m, by = c("datetime", "var_ref_id", "site", "device_id", "variable")) |> 
  mutate(
    qc_code = case_when(
      !is.na(qc_value) ~ "QC 300",
      is.na(raw_value) ~ "QC 100",
...
met <- raw_long_device_filt |> 
  filter(variable %in% c("pp_rain", "pr_baro", "r_swd", "t_air", "h_rh", 
                         "w_dir", "w_spd")) |> 
...
Download the data as a csv file. The data is in long format, with the variable 
names in the `variable` column and the raw values in the `raw_value` column and
the QC value `qc_value`. The `qc_code` column contains the quality control codes 
...
write_csv(data, file = file.path(path, "rotoehu_qc.csv"), na = "")
