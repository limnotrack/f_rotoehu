...
[GitHub](https://github.com/limnotrack/f_rotoehu/releases/tag/v0.0.1) or within
...
# Delete raw data
unlink("data-raw/rotoehu_sftp_compiled_raw.csv", force = TRUE)
```
...
```{r}
#| label: reassign-variables-do-cont0p5m-1
#| warning: false
#| message: false
...
```{r}
#| label: reassign-variables-do-cont0p5m-2
#| warning: false
#| message: false
...
```{r}
#| label: reassign-variables-do-cont10m-1
#| warning: false
#| message: false
...
```{r}
#| label: fig-rh-drift-yearly
#| warning: false
#| message: false
#| fig.cap: Relative humidity drift by year.
...
codes are from the 
[National Environmental Monitoring Standards (NEMS)](https://www.nems.org.nz/). 
...
fluorescence or phycocyanin fluorescence sensors. We have therefore
adapted the NEMS quality control codes to include these sensors, using similar
checks and adjustments described in the [Processing of Environmental Time-series Data](https://www.nems.org.nz/documents/data-processing) (v1.2.0, August 2024) 
document.
...
#          value = "qc_value")
# var_ref_id <- "c_ph_d100"

# data <- ph <- raw_long_device |> 
#   filter(var_abbr %in% c("c_ph"))

ph_count <- raw_long_device |> 
  filter(var_abbr %in% c("c_ph_raw", "c_ph")) |>
  apply_adjustment(var_ref_id = "c_ph_raw_d100", FUN = \(x) x / 10) |> 
...
# plotly_data(data = ph_count, y1 = "c_ph_d100", y2 = "c_ph_raw_d100", sub = 20)
...
Chlorophyll and phycocyanin flurorescence sensors currently do not have a 
NEMS classification. We will apply quality control methods to these data and 
will use the same QC codes as other variables.
...
data |> 
  dplyr::summarise(n = dplyr::n(), .by = c(datetime, site, var_ref_id)) |>
  dplyr::filter(n > 1L)
data_qc_wide <- data |> 
...
zip(zipfile = "rotoehu_data_qc.zip", files = path)
...
<a href="https://github.com/limnotrack/f_rotoehu/releases/download/v0.0.1/rotoehu_data_qc.zip" download class="btn btn-primary">Download Data</a>
